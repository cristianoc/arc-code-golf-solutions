    ...
    for i in range(H):
        for j in range(W):
            if G[i][j] and not vis[i][j]:
                ...
                while q:
                    a,b=q.pop()
                    v=G[a][b]; pal.add(v); cells.append((a,b,v))
                    for da,db in ((1,0),(-1,0),(0,1),(0,-1)):
                        x,y=a+da,b+db
                        if 0<=x<H and 0<=y<W and G[x][y] and not vis[x][y]:
                            vis[x][y]=True; q.append((x,y))
...
def mt(G,cs):
    H,W=len(G),len(G[0])
    hi=max(i for i,_,_ in cs)+1; wj=max(j for _,j,_ in cs)+1
    out=[]
...
def p(g):
    ...
    for x in ns:
        ...
        for f in T:
            ...
            mi=min(i for i,_,_ in F); mj=min(j for _,j,_ in F)
            F=[(i-mi,j-mj,v) for i,j,v in F]
            a=[(f(i,j)[0],f(i,j)[1],v) for i,j,v in A]
            a=[(i-mi,j-mj,v) for i,j,v in a]
            ai=min(i for i,_,_ in a); aj=min(j for _,j,_ in a)
