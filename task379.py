def p(g):
    ...
    R=range; h,w=len(g),len(g[0]); O=[r[:] for r in g]
    EIGHT=8; RED=2
        ...
        for i,j in reds:
            if len(rails)>=2:
                ...
                if lo is not None and up is not None and lo!=up:
                    ...
                    O[lo][j]=O[up][j]=RED; square(lo,j); square(up,j); continue
            rr=min(rails,key=lambda r:abs(r-i))
            d=1 if rr>i else -1; k=i
            while True:
                nk=k+d
                if (d==1 and nk>=rr-1) or (d==-1 and nk<=rr+1): break
                k=nk; O[k][j]=RED
            O[rr][j]=RED; square(rr,j)
    else:
        rails=sorted(cols)
        if not rails: return O
        for i,j in reds:
            if len(rails)>=2:
                ...
                if le is not None and ri is not None and le!=ri:
                    ...
                    O[i][le]=O[i][ri]=RED; square(i,le); square(i,ri); continue
            rc=min(rails,key=lambda c:abs(c-j))
            d=1 if rc>j else -1; k=j
            while True:
                nk=k+d
                if (d==1 and nk>=rc-1) or (d==-1 and nk<=rc+1): break
                k=nk; O[i][k]=RED
            O[i][rc]=RED; square(i,rc)
    return O
