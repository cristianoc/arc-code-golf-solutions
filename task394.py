...
def solve_f9012d9b(I):
    ...
    r1=c1=r2=c2=None
    if zeros:
        r1=min(i for i,_ in zeros); r2=max(i for i,_ in zeros)
        c1=min(j for _,j in zeros); c2=max(j for _,j in zeros)
        zh=r2-r1+1; zw=c2-c1+1
        ...
        for part in _vsplit(I, n):
            if ZERO not in _palette(part):
                ...
                if best_h is None or score > best_h_score:
                    best_h = part; best_h_score=score
    if best_v is None:
        parts=_hsplit(I,2)
